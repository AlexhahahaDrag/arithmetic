...
public class MaximumBobPoints {
    ...
    public int[] maximumBobPoints(int numArrows, int[] aliceArrows) {
        ...
        dfs(aliceArrows, numArrows, res,11, 0);
        return result;
    }

    private void dfs(int[] aliceArrows, int numArrows, int[] res, int index, int sum) {
        //没有箭或者位移小于0，就返回
        ...
        if (index == 0) {
            if (sum > max) {
                max = sum;
                result = Arrays.copyOfRange(res, 0 , res.length);
                result[0] = numArrows;
            }
            return;
        }
        //不取当前值
        dfs(aliceArrows, numArrows, res, index - 1, sum);
        //如果剩余箭数大于当前位置alice的箭数
        if (numArrows >= aliceArrows[index] + 1) {
            res[index] = aliceArrows[index] + 1;
            dfs(aliceArrows, numArrows - aliceArrows[index] - 1, res, index - 1, sum + index);
            res[index] = 0;
        }
    }
    ...
    public static void main(String[] args) {
        ...
        int[] res = maximumBobPoints.maximumBobPoints(numArrows, aliceArrows);
        for(int i : res) {
            System.out.print(i + ",");
        }
    }
