...
public class NumDecodings {
    ...
    public int numDecodings(String s) {
        ...
        for (int i = 1; i < st.length; i++) {
            if (st[i] == '0') {
                ...
                dp[i] = i - 2 < 0 ? 1 : dp[i - 2];
            } else if(st[i - 1] == '1' || (st[i - 1] == '2' && st[i] <= '6')) {
    ...
    private void find(char[] st, int i) {
        if (i >= st.length) {
            res++;
            return;
        }
        if(st[i] == '0') {
            return;
        } else if (i + 1 < st.length && ((st[i] == '2' && st[i + 1] <= '6') || st[i] == '1')){
            find(st, i + 2);
        }
        find(st, i + 1);
    }
...
//        String s = "10011";
        ...
        System.out.println(numDecodings.numDecodings1(s));
    }
}
