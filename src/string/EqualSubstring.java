...
public class EqualSubstring {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(equalSubstring.equalSubstring("abcd", "cdef", 2));
    }
    ...
    private int equalSubstring(String s, String t, int maxCost) {
        ...
        while(start < len) {
            sum += map[start++];
            while (sum > maxCost) {
                max = max > start - position - 1 ? max : start - position - 1;
                sum -= map[position++];
            }
        }
        return max = max > start - position ? max : start - position;
    }
}
